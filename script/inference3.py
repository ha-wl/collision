...
class Args:
    def __init__(self):
        ...
        self.pretrained_model: str = "../outputs/sonoisa__t5-base-japanese/2024-01-21/14-13-19"
...
def main(args: Args):
    ...
    plain_dataset: list[dict] = load_dataset(args.dataset_dir / "plain4.jsonl")
    em_model = embedding.SentenceT5(args.pretrained_model, args.model_name)
    dist_model = keras.models.load_model('../outputs/dist_model/2024-01-21/14-13-19')
    weightspeed_model = keras.models.load_model('../outputs/weightspeed_model/2024-01-21/14-13-19')
    ...
    def collate_fn(dataset: list) -> BatchEncoding:
        ...
        return ids, patterns, weightspeeds, distributions, inputs, input, labels 
    ...
    def evaluate(dataloader: DataLoader):
        ...
        for batch in tqdm(dataloader, total=len(dataloader), dynamic_ncols=True, leave=False):
            ids, patterns, weightspeeds, distributions, input_ids, input, outputs = batch
        ...
        for i in range(len(id_list)):
            output_list.append({"bleu": bleu_scores/data_size, "ROUGE-2":rouge2_scores/data_size, "ROUGE-L":rougel_scores/data_size,
                                "ID":id_list[i], "pattern":pattern_list[i], "pred":preds[i], "weightspeed_pred":weightspeed_pred[i], "weightspeed_lst":weightspeed_list[i], "dist_pred":dist_pred[i], "dist_lst":distribution_list[i]}) #"gold": golds[:, i]
