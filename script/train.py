...
class Args:
    def __init__(self): #学習データの初期化
        ...
        self.device: str =  "cuda:3"  #ここを変更するべき？
        self.seed: int = 42
...
def main(args: Args):
    ...
    train_dataset: list[dict] = load_dataset(args.dataset_dir / "train1.jsonl")
    val_dataset: list[dict] = load_dataset(args.dataset_dir / "val1.jsonl")
    test_dataset: list[dict] = load_dataset(args.dataset_dir / "test1-1.jsonl")
    ...
    for epoch in trange(args.epochs, dynamic_ncols=True):
        ...
        for batch in tqdm(
            ...
            ids, pattern, input_ids, outputs = batch
