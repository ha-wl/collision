...
class Args:
    def __init__(self):
        ...
        self.model_date: str = ("2024-12-10/21-42-01")
        self.pretrained_model: str = Path("../outputs/sonoisa__t5-base-japanese")/ self.model_date
        ...
        self.nnmodel_dir: Path = Path("../outputs/nn_model")
...
def main(args: Args):
    ...
    def collate_fn(dataset: list) -> BatchEncoding:
        ...
        return ids, patterns, atodukes, weightspeeds, distributions, inputs, labels, teido_nashis, kigous 
    ...
    def evaluate(dataloader: DataLoader):
        model.eval()
        # loss = 0
        gold_labels, pred_labels, id_list, pattern_list, atoduke_list, weightspeed_list, distribution_list, teido_nashi_list, kigou_list = [], [], [], [], [], [], [], [], []
        output_list = []
        ...
        for batch in tqdm(dataloader, total=len(dataloader), dynamic_ncols=True, leave=False):
            ids, patterns, atodukes, weightspeeds, distributions, input_ids, outputs, teido_nashis, kigous = batch
        ...
        nn_pred = nn_model.predict(seikai)

        for i in range(len(id_list)):
            if kigou_list[i] ==1: #予測がteido_nashiより大きくなる
                if nn_pred[i][0] >= teido_nashi_list[i][0]:
                    seigo = 1
                else :
                    seigo = 2
            elif kigou_list[i] ==2:
            ...
            output_list.append({"bleu": bleu_scores/data_size, "ROUGE-2":rouge2_scores/data_size, "ROUGE-L":rougel_scores/data_size,
                                "ID":id_list[i], "pattern":pattern_list[i],"atoduke":atoduke_list[i], "pred":preds[i],  "dist_pred":nn_pred[i], "dist_lst":distribution_list[i],"teido_nashi":teido_nashi_list[i],"kigou":kigou_list[i], "seigo":seigo}) #"gold": golds[:, i]
    ...
    utils.save_jsonl(plain_metrics, args.output_dir / "plain-metrics.jsonl")
