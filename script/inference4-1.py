...
def main(args: Args):
    ...
    def collate_fn(dataset: list) -> BatchEncoding:
        ...
        return ids, patterns, atodukes, distributions, inputs, labels, teido_nashis, kigous 
    ...
    def evaluate(dataloader: DataLoader):
        model.eval()
        # loss = 0
        gold_labels, pred_labels, id_list, pattern_list, atoduke_list, distribution_list, teido_nashi_list, kigou_list = [], [], [], [], [], [], [], []
        output_list = []
        ...
        for batch in tqdm(dataloader, total=len(dataloader), dynamic_ncols=True, leave=False):
            ids, patterns, atodukes, distributions, input_ids, outputs, teido_nashis, kigous = batch
