...
class Args:
    def __init__(self):
        ...
        self.pretrained_model: str = "../outputs/sonoisa__t5-base-japanese/2024-02-12/18-06-39"
...
def main(args: Args):
    ...
    plain_dataset: list[dict] = load_dataset(args.dataset_dir / "plain2.jsonl")
    em_model = embedding.SentenceT5(args.pretrained_model, args.model_name)
    nn_model = keras.models.load_model('../outputs/nn_model/2024-02-12/18-06-39')
    ...
    def collate_fn(dataset: list) -> BatchEncoding:
        ...
        return ids, patterns, distributions, inputs, labels 
    ...
    def evaluate(dataloader: DataLoader):
        ...
        for batch in tqdm(dataloader, total=len(dataloader), dynamic_ncols=True, leave=False):
            ids, patterns, distributions, input_ids, outputs = batch
        ...
        for i in range(len(id_list)):
            output_list.append({"bleu": bleu_scores/data_size, "ROUGE-2":rouge2_scores/data_size, "ROUGE-L":rougel_scores/data_size,
                                "ID":id_list[i], "pattern":pattern_list[i], "pred":preds[i],  "dist_pred":nn_pred[i], "dist_lst":distribution_list[i]}) #"gold": golds[:, i]
